...
export async function updateContent(buttonInfo, contentContainer) {
    ...
    const sanitizeConfig = {
        ADD_URI_SCHEMES: ['chrome-extension', 'moz-extension'],
    };
...
function initializeHeartbeatSpoofer() {
    ...
    window.fetch = async function (...args) {
        ...
        if (args.length > 1 && args[1] && args[1].headers) {
            ...
            if (
                typeof newOptions.headers.get === 'function' &&
                typeof newOptions.headers.forEach === 'function'
            ) {
                if (newOptions.headers.get('RoValra-Internal') === 'true') {
                    hasHeader = true;
                    const copiedHeaders = {};
                    newOptions.headers.forEach((headerValue, headerName) => {
                        if (headerName.toLowerCase() !== 'rovalra-internal') {
                            copiedHeaders[headerName] = headerValue;
                        }
                    });
                    newOptions.headers = copiedHeaders;
                }
            } else if (
