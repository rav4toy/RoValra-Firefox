...
(function() {
  ...
  function dispatchCaptureEvent(url, method, body) {
    typeof url == "string" && (url.match(
      /\.(js|css|png|jpg|jpeg|gif|ico|svg|woff|woff2|ttf|eot|mp3|ogg|wav|webm|mp4|json)$/i
    ) && !url.includes("apis.roblox.com") && !url.includes("games.roblox.com") || !url.includes("roblox.com") && !url.includes("rovalra.com") || document.dispatchEvent(
  ...
  window.fetch = async function(...args) {
    ...
    return typeof url == "string" && (url.includes(CATALOG_API_URL) && response.clone().json().then(
    ...
    }), url.includes(CATALOG_API_URL) && response.clone().json().then(
    ...
    }), url.includes(CLIENT_STATUS_API_URL) && response.clone().json().then(
      (d) => document.dispatchEvent(
    ...
    }), url.includes(GAME_LAUNCH_SUCCESS_URL) && url.includes("GameLaunchSuccessWeb_Win32") && document.dispatchEvent(
      new CustomEvent("rovalra-game-launch-success", {
        detail: { url }
      })
    ), url.includes(GAME_SERVERS_API_URL) && url.includes("/servers/") && response.clone().json().then(
      (d) => document.dispatchEvent(
    ...
    }), url.includes(GAMES_ROBLOX_API) && url.includes("/media") && response.clone().json().then(
      (d) => document.dispatchEvent(
    ...
    }), url.includes(TRADES_API_URL) && url.includes("/tradableitems") && response.clone().json().then(
      (d) => document.dispatchEvent(
    ...
    }), url.includes(TRADES_LIST_API_URL) && response.clone().json().then(
      (d) => document.dispatchEvent(
      ...
      if (typeof xhr._rovalra_url == "string") {
        ...
        try {
          ...
          url.includes(CATALOG_API_URL) && window.dispatchEvent(
            ...
            JSON.parse(xhr.responseText)
          ), url.includes(GAME_SERVERS_API_URL) && url.includes("/servers/") && triggerEvent("rovalra-game-servers-response", {
            url,
