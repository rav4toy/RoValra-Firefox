...
async function fetchLatestPlaceVersion(placeId) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Roblox API request failed with status: ${response.status}`,
            );
        }
...
export async function fetchServerStats(placeId) {
    ...
    try {
        ...
        if (
            data.counts.regions &&
            typeof data.counts.total_servers !== 'number'
        ) {
            data.counts.total_servers = Object.values(
                data.counts.regions,
...
function applyVersionAttributes(element) {
    ...
    const serverItemContainer =
        element ||
        document.getElementById('rbx-public-game-server-item-container') ||
        document.querySelector(
            '.rovalra-modern-ui > .flex.flex-col:not(.gap-xsmall)',
        );
    if (
        serverItemContainer &&
        !serverItemContainer.hasAttribute('data-newest-version')
    ) {
        if (versionDataCache.newest_place_version) {
            serverItemContainer.dataset.newestVersion =
                versionDataCache.newest_place_version;
        }
        if (versionDataCache.oldest_place_version) {
            serverItemContainer.dataset.oldestVersion =
                versionDataCache.oldest_place_version;
        }
    }
}
...
async function createStatsBarUI(serverListContainer) {
    ...
    const header =
        serverListContainer.querySelector('.container-header') ||
        serverListContainer.querySelector('.flex.flex-col.gap-xsmall');
    ...
    statsContainer.style.cssText =
        'display: flex; gap: 0; margin-bottom: 9px; margin-left: 9px; gap: 4.5px; align-items: center;';
    ...
    if (
        settings.OldestVersionEnabled === true &&
        counts.oldest_place_version &&
        hasActiveStats
    ) {
    ...
    if (
        serverListContainer.closest(
            '#roseal-running-game-instances-container',
        ) &&
        !serverListContainer.classList.contains('rovalra-modern-ui')
    ) {
        const controls = document.getElementById('rovalra-main-controls');
        const options = serverListContainer.querySelector(
            '.server-list-options',
        );
...
export async function initGlobalStatsBar() {
    ...
    observeElement(
        '.rovalra-modern-ui[data-rovalra-section-type="public"] > .flex.flex-col:not(.gap-xsmall)',
        (element) => {
            applyVersionAttributes(element);
        },
    );
    ...
    observeElement('.rovalra-modern-ui', (element) => {
        if (
            versionDataCache &&
            element.dataset.rovalraSectionType === 'public'
        ) {
            createStatsBarUI(element);
        }
    });
    ...
    try {
        ...
        if (data) {
            ...
            document
                .querySelectorAll(
                    '.rovalra-modern-ui[data-rovalra-section-type="public"]',
                )
                .forEach((element) => {
                    createStatsBarUI(element);
                });
