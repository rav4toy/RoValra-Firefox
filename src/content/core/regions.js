...
async function fetchAndProcessRegions() {
    ...
    await CacheHandler.set('regions', STORAGE_KEY_REGIONS, newRegions, 'local');
    await CacheHandler.set(
        'regions',
        STORAGE_KEY_CONTINENTS,
        newContinents,
...
export async function getRegionData() {
    ...
    const cachedRegions = await CacheHandler.get(
        'regions',
        STORAGE_KEY_REGIONS,
        'local',
    );
    const cachedContinents = await CacheHandler.get(
        'regions',
        STORAGE_KEY_CONTINENTS,
        'local',
    );
    ...
    if (
        cachedRegions &&
        cachedRegions['AUTO'] &&
        cachedRegions['AUTO'].city === 'Automatic'
    ) {
        for (const [code, info] of Object.entries(cachedRegions)) {
            info.id = code;
        }
        REGIONS = cachedRegions;
        cachedRegionData = {
            regions: cachedRegions,
            continents: cachedContinents || {},
        };
        return cachedRegionData;
    } else {
        try {
