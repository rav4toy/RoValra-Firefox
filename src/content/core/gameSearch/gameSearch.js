...
export async function searchGames(query, sessionId) {
    ...
    try {
        ...
        const gameContents =
            results.searchResults?.flatMap((group) =>
                group.contentGroupType === 'Game' ? group.contents : [],
            ) || [];
        const topGames = gameContents.slice(0, 3);

        if (topGames.length > 0) {
            const thumbnailItems = topGames.map((game) => ({
                id: game.universeId,
            }));
            const thumbnails = await fetchThumbnailsBatch(
                thumbnailItems,
                'GameIcon',
                '50x50',
                false,
...
export async function searchGroups(query) {
    try {
        const response = await callRobloxApi({
            subdomain: 'groups',
            endpoint: `/v1/groups/search?cursor=&keyword=${encodeURIComponent(query)}&limit=25&prioritizeExactMatch=true&sortOrder=Asc`,
        });
        if (!response.ok) return [];

        const results = await response.json();
        const groups = (results.data || []).slice(0, 8);
        const thumbnails = await fetchThumbnailsBatch(
        ...
        return groups.map((group) => ({
            ...group,
            thumbnail:
                thumbnails.get(Number(group.id)) ||
