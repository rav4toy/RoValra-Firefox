...
// --- Context Menu (Firefox uses browser.menus instead of chrome.contextMenus) ---
...
async function setupContextMenuListener() {
    const hasRequiredPermissions = await chrome.permissions.contains({ permissions: ['menus'] });
    if (hasRequiredPermissions && chrome.menus && !chrome.menus.onClicked.hasListener(contextMenuClickListener)) {
...
// --- Proxy Fetch (Firefox: content scripts can't use fetch for cross-origin, route via background) ---

async function proxyFetch(request) {
    try {
        let { url, method = 'GET', headers = {}, body = null, credentials = 'omit' } = request;
        ...
        return { ok: false, status: 0, statusText: e.message, headers: {}, body: null, error: e.message };
    }
}
...
chrome.permissions.onRemoved.addListener((permissions) => {
    ...
    if (permissions.permissions?.includes('menus') && chrome.menus?.onClicked.hasListener(contextMenuClickListener)) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    switch (request.action) {
        ...
        case 'proxyFetch':
            (async () => {
                try {
                    ...
                    sendResponse({ ok: false, status: 0, statusText: e.message, headers: {}, body: null, error: e.message });
                }
            })();
            return true;
        ...
        case 'requestPermission':
            // Firefox requires permissions.request() to be called from a user gesture context.
            // We open a popup page that the user interacts with to grant the permission.
            ...
            (async () => {
                ...
                const pageUrl = chrome.runtime.getURL('public/Assets/permission_request.html?permission=' + encodeURIComponent(permission[0]) + '&requestId=' + requestId);
                const resultPromise = new Promise((resolve) => {
                    ...
                    setTimeout(() => { chrome.runtime.onMessage.removeListener(resultListener); resolve(false); }, 120000);
                });
