...
async function callRobloxApiBackground(options) {
    ...
    const urlBase = fullUrl || `${baseUrl}${endpoint}`;

    let isStaticBinaryRequest = false;
    try {
        isStaticBinaryRequest = /\.(?:glb|gltf|bin|png|jpe?g|webp|gif|bmp|svg|ktx2?|hdr|mp3|ogg|wav)(?:$|[?#])/i.test(
            new URL(urlBase).pathname,
        );
    } catch (e) {}

    const shouldAppendMarker =
        !urlBase.includes('_RoValraRequest=') &&
        !endpoint.includes('/player-hydration-service/v1/players/signed') &&
        !isStaticBinaryRequest;
    const url = shouldAppendMarker
        ? `${urlBase}${urlBase.includes('?') ? '&' : '?'}_RoValraRequest=`
        : urlBase;

    const fetchHeaders = new Headers(headers || {});
    if (!fetchHeaders.has('Accept')) {
        fetchHeaders.set('Accept', isStaticBinaryRequest ? '*/*' : 'application/json');
    }
    ...
    if (noCache) fetchOptions.cache = 'no-store';
    ...
    if (!isRovalraApi && method !== 'GET' && method !== 'HEAD' && state.csrfTokenCache) {
...
chrome.permissions.onAdded.addListener((permissions) => {
    ...
    if (permissions.permissions?.includes('menus'))
        setupContextMenuListener();
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    switch (request.action) {
            ...
            (async () => {
                try {
                    ...
                    const response = await callRobloxApiBackground({
                        ...
                        isRovalraApi: /(^|\.)rovalra\.com$/i.test(
                    ...
                    sendResponse({ ok: true, contentType, body: bodyBase64, bodyBase64 });
                } catch (error) {
        ...
        case 'fetchAssetAsDataUrl':
            fetchAssetAsDataUrl(request.url)
